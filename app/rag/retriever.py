...
async def retriever_query(input: dict):
     question = input.get("question")
     doc_id = input.get("doc_id")
     docs = await retriever(question, doc_id=doc_id) #get the relevant chunks using the retriever
     # docs = await reranker(docs, question) rerank the chunks using the reranker
     return docs

async def retriever(question, doc_id=None, k=3):
